...
.shell { width:100%; max-width:418px; min-height:100%; position:relative; overflow-y:auto; overflow-x:hidden; scrollbar-width:none;
...
/* ---- home carousel ---- */
.hcar { display:flex; flex:1; min-height:0; gap:11px; overflow-x:auto; scroll-snap-type:x mandatory; scroll-behavior:smooth;
  margin:0 -13px; padding:3px 13px 5px; scrollbar-width:none; -ms-overflow-style:none; }
.hcar::-webkit-scrollbar { display:none; }
.hcard { scroll-snap-align:center; flex:0 0 100%; min-width:0; height:100%; border-radius:26px; padding:12px 10px;
  display:flex; flex-direction:column; gap:9px; text-align:left; position:relative; overflow:hidden;
  background:linear-gradient(158deg, rgba(255,255,255,.15), rgba(255,255,255,.035) 48%), var(--glass);
  border:1px solid var(--line); box-shadow:var(--drop), var(--rim);
  -webkit-backdrop-filter:blur(26px) saturate(185%); backdrop-filter:blur(26px) saturate(185%); }
.hart { width:100%; flex:1; min-height:76px; border-radius:19px; position:relative; display:flex; align-items:center;
  justify-content:center; border:1px solid var(--line2); overflow:hidden;
  background:radial-gradient(closest-side at 50% 44%, rgba(43,123,255,.4), transparent 78%),
    linear-gradient(160deg, rgba(255,255,255,.09), transparent 55%); }
.hart::after { content:''; position:absolute; left:24%; right:24%; bottom:10%; height:8px; border-radius:50%;
  background:radial-gradient(closest-side, rgba(79,216,255,.4), transparent 74%); }
.hcard .ht { font-family:var(--display); font-weight:800; letter-spacing:-.02em; font-size:19px; }
.hcard .hm { font-family:var(--mono); font-size:10px; color:var(--dim); letter-spacing:.06em;
  line-height:1.5; margin-top:4px; }
.hcard .cta, .hcard .ghost { width:auto; align-self:stretch; margin:0 -7px; min-height:48px; padding:14px 14px; }
.hcard .hart, .hcard > div:not([class]) { margin:0 4px; }
.dots { display:flex; gap:6px; align-items:center; justify-content:center; margin-top:11px; }
.dots i { width:6px; height:6px; border-radius:999px; background:var(--line); transition:.28s; }
.dots i[data-on="1"] { width:20px; background:linear-gradient(90deg,#8FEBFF,#2B7BFF); }
.carnav { min-height:44px; border-radius:999px; flex:1 1 0; min-width:0; padding:7px 12px; color:var(--ice);
  ...
  -webkit-backdrop-filter:blur(16px); backdrop-filter:blur(16px); transition:.22s; }
.carnav:hover { background:var(--glass2); }
.carnav:active { transform:scale(.96); }
.carnav[disabled] { opacity:.32; }
.carnav .cn-g { font-size:19px; line-height:1; flex-shrink:0; color:var(--ice); }
.carnav .cn-l { min-width:0; }
.carnav .cn-k { display:block; font-family:var(--mono); font-size:8px; letter-spacing:.2em;
  text-transform:uppercase; color:var(--dim); }
.carnav .cn-n { display:block; font-family:var(--display); font-weight:700; font-size:11.5px;
  letter-spacing:.05em; text-transform:uppercase; white-space:nowrap; overflow:hidden;
  text-overflow:ellipsis; margin-top:2px; }
.trash { width:34px; height:34px; border-radius:50%; flex-shrink:0; display:flex; align-items:center;
  justify-content:center; background:rgba(255,122,92,.1); border:1px solid rgba(255,122,92,.32);
  color:var(--heat); transition:.22s; }
.trash:hover { background:rgba(255,122,92,.2); }
.trash:active { transform:scale(.9); }

/* frosted sheen on the glass surfaces */
.prod::before, .bl::before, .row::before, .card::before, .hcard::before {
  content:''; position:absolute; inset:0; border-radius:inherit; pointer-events:none;
  background:linear-gradient(158deg, rgba(255,255,255,.13), rgba(255,255,255,0) 46%); }
.prod, .bl, .row, .card { position:relative; }
...
/* ---- top bar ---- */
.top { display:flex; align-items:center; gap:9px; padding:11px 13px; position:sticky; top:0; z-index:20;
...
/* ---- layout ---- */
.body { flex:1; padding:12px 13px 20px; position:relative; z-index:2; }
...
.cta { width:100%; padding:17px 16px; border-radius:999px; font-size:15px; color:#03101E;
...
.ghost { width:100%; padding:15px 16px; border-radius:999px; color:var(--ice); font-size:13.5px; font-weight:600;
  background:var(--glass); border:1px solid var(--line); box-shadow:var(--rim);
...
/* ---- main menu ---- */
.mainmenu { flex:1; display:flex; flex-direction:column; padding:14px 13px 12px; position:relative; z-index:2; }
.wm { font-size:clamp(34px,11vw,46px); line-height:.92; text-transform:uppercase; margin:0; letter-spacing:-.035em; }
...
.row { display:flex; align-items:center; gap:11px; padding:12px; margin-bottom:8px; border-radius:22px;
  background:var(--glass); border:1px solid var(--line); box-shadow:var(--drop), var(--rim);
  -webkit-backdrop-filter:blur(22px) saturate(170%); backdrop-filter:blur(22px) saturate(170%); }
...
.bl { display:flex; gap:11px; padding:11px; margin-bottom:9px; border-radius:22px; align-items:center;
  background:var(--glass); border:1px solid var(--line); box-shadow:var(--drop), var(--rim);
  -webkit-backdrop-filter:blur(22px); backdrop-filter:blur(22px); }
.bl-img { width:56px; height:56px; flex-shrink:0; border-radius:18px; display:flex; align-items:center;
...
const PATHS = {
  ...
  play: "M8 4.5l12 7.5-12 7.5z",
  flame: "M12 22a6 6 0 006-6c0-5-6-13-6-13S6 11 6 16a6 6 0 006 6zM12 19a3 3 0 003-3c0-2-3-5-3-5s-3 3-3 5a3 3 0 003 3z",
  user: "M12 12a4 4 0 100-8 4 4 0 000 8zM4 21c0-4.2 3.6-6.5 8-6.5s8 2.3 8 6.5",
  bag: "M4 7h16l-1.6 14H5.6zM9 7a3 3 0 016 0",
...
const PFP = [["#8FEBFF", "#2B7BFF"], ["#B79BFF", "#6C3BFF"], ["#FFD98A", "#FF7A45"],
  ["#8BF0C0", "#22B573"], ["#FF9EC0", "#8E5CFF"], ["#DCE6F7", "#7C8BA6"]];

const Avatar = ({ i, size = 54, label }) => (
  <div style={{
    width: size, height: size, borderRadius: "32%", flexShrink: 0,
    display: "flex", alignItems: "center", justifyContent: "center",
    background: `linear-gradient(140deg,${PFP[i][0]},${PFP[i][1]})`, color: "#04101F",
    fontFamily: "var(--display)", fontWeight: 800, fontSize: Math.round(size * 0.42), letterSpacing: "-.02em",
    boxShadow: "inset 0 1px 0 rgba(255,255,255,.55), 0 10px 24px rgba(0,0,0,.45)",
  }}>{label}</div>
);

/* rough calorie estimates — a real build would call a nutrition API */
const FOOD_DB = [
  { k: ["oat", "porridge", "muesli"], t: "Oats with milk", kcal: 380 },
  { k: ["chicken wrap", "wrap"], t: "Chicken wrap", kcal: 520 },
  { k: ["chicken"], t: "Grilled chicken and rice", kcal: 610 },
  { k: ["burger", "big mac", "cheeseburger"], t: "Cheeseburger", kcal: 690 },
  { k: ["pizza"], t: "Pizza, two slices", kcal: 580 },
  { k: ["pasta", "spaghetti", "bolognese"], t: "Pasta bolognese", kcal: 720 },
  { k: ["salad"], t: "Chicken salad", kcal: 320 },
  { k: ["sandwich", "sanga", "toast"], t: "Sandwich", kcal: 420 },
  { k: ["protein shake", "shake", "whey"], t: "Protein shake", kcal: 210 },
  { k: ["coke", "soft drink", "pepsi", "soda"], t: "Soft drink, 600 ml", kcal: 250 },
  { k: ["energy drink", "monster", "red bull"], t: "Energy drink", kcal: 160 },
  { k: ["coffee", "latte", "flat white"], t: "Flat white", kcal: 120 },
  { k: ["banana", "apple", "fruit"], t: "Piece of fruit", kcal: 105 },
  { k: ["rice"], t: "Rice, one cup", kcal: 205 },
  { k: ["egg"], t: "Two eggs on toast", kcal: 340 },
  { k: ["chips", "fries", "hot chips"], t: "Hot chips", kcal: 480 },
  { k: ["sushi"], t: "Sushi, three rolls", kcal: 390 },
  { k: ["ice cream", "chocolate", "lolly", "dessert"], t: "Chocolate / dessert", kcal: 350 },
];
const SCANS = [
  { t: "Chicken and rice bowl", kcal: 610, conf: 92 },
  { t: "Cheeseburger and chips", kcal: 940, conf: 88 },
  { t: "Protein shake, 500 ml", kcal: 210, conf: 95 },
  { t: "Pizza, two slices", kcal: 580, conf: 84 },
  { t: "Chicken salad", kcal: 320, conf: 91 },
];
const BURNS = [
  { t: "Push-up station", per: 8.5, xp: 55, kit: "Push-up bars" },
  { t: "Band circuit", per: 7.2, xp: 48, kit: "Resistance bands" },
  { t: "Squat holds", per: 6.4, xp: 40, kit: "Bodyweight" },
  { t: "Skipping intervals", per: 11.5, xp: 70, kit: "Bodyweight" },
];
const ACT = [[1.25, "Low"], [1.45, "Medium"], [1.7, "High"]];
const GOALS = {
  cut: { d: -500, l: "Lose fat", r: "about \u22120.45 kg per week", burn: 320 },
  maintain: { d: 0, l: "Maintain", r: "hold your current weight", burn: 200 },
  bulk: { d: 320, l: "Build muscle", r: "about +0.25 kg per week", burn: 140 },
};
...
function LevelUpPrototype() {
  ...
  const [prof, setProf] = useState({ name: "Boral", pfp: 0, height: 178, weight: 74, age: 17, sex: "m", act: 1.45, goal: "cut" });
  const [meals, setMeals] = useState([
    { id: "d1", t: "Overnight oats, banana", kcal: 410, src: "described" },
    { id: "d2", t: "Chicken wrap", kcal: 520, src: "photo" },
  ]);
  const [burned, setBurned] = useState(180);
  const [foodQ, setFoodQ] = useState("");
  const [nutriTab, setNutriTab] = useState("describe");
  ...
  /* ---- nutrition maths ---- */
  const initial = (prof.name || "P").trim().charAt(0).toUpperCase() || "P";
  const bmr = Math.round(10 * prof.weight + 6.25 * prof.height - 5 * prof.age + (prof.sex === "m" ? 5 : -161));
  const tdee = Math.round(bmr * prof.act);
  const goalDef = GOALS[prof.goal];
  const kcalTarget = tdee + goalDef.d;
  const eaten = meals.reduce((a, m) => a + m.kcal, 0);
  const allowance = kcalTarget + burned - eaten;
  const burnGoal = goalDef.burn;
  const burnLeft = Math.max(0, burnGoal - burned) + Math.max(0, -allowance);

  const estimateFood = (q) => {
    const t = q.trim().toLowerCase();
    if (!t) return null;
    const hit = FOOD_DB.find((f) => f.k.some((w) => t.includes(w)));
    return hit
      ? { t: hit.t, kcal: hit.kcal, src: "described", conf: null }
      : { t: q.trim().slice(0, 30), kcal: 250, src: "described", conf: null, rough: true };
  };
  const logMeal = (m) => {
    setMeals((ms) => [...ms, { ...m, id: `m${Date.now()}` }]);
    setPending(null); setFoodQ("");
    flash(`Logged ${nf(m.kcal)} kcal`);
  };
  const takePhoto = () => {
    setScanning(true);
    setTimeout(() => {
      ...
      setScanning(false);
      setPending({ ...pick, src: "photo" });
    }, 900);
  };
  const logBurn = (b, mins) => {
    const kcal = Math.round(b.per * mins);
    setBurned((v) => v + kcal);
    setP((q) => ({ ...q, xp: q.xp + b.xp, week: q.week + b.xp }));
    flash(`${b.t} logged · ${kcal} kcal · +${b.xp} XP`);
  };
  ...
  const brandHead = (
    <div className="reveal" style={{ animationDelay: "40ms", marginBottom: 11 }}>
      <h1 className="wm">Level<em>up</em></h1>
      <div className="tagline">Train · Rank · Cash out</div>
      <div style={{ marginTop: 7 }}>
        <span className="chip" style={{ color: "var(--gold)", fontSize: 7.6, letterSpacing: ".08em", whiteSpace: "nowrap" }}>Interactive preview · resets on refresh</span>
      </div>
    </div>
  );
  ...
  const rankCard = (
    <Cut>
      <div className="opcard">
        <Avatar i={prof.pfp} size={54} label={initial} />
        <div>
          <div className="hname">{prof.name || "Player"}</div>
  ...
  const menuOpts = [
    { k: "profile", n: "Profile", m: `${prof.height} cm · ${prof.weight} kg · ${goalDef.l.toLowerCase()}` },
    { k: "nutri", n: "Calories", m: allowance >= 0 ? `${nf(allowance)} kcal left today` : `${nf(-allowance)} kcal over target` },
  ];

  const HOME = [
    { k: "hub", n: "Play", tag: "session ready", ic: "play", c: "var(--cyan)", primary: true, b: "▶ Play",
      m: `${openQ ? `${openQ} quest${openQ > 1 ? "s" : ""} open` : "all quests cleared"} · ${nf(p.week)} XP this week` },
    { k: "nutri", n: "Track calories", tag: "nutrition", ic: "flame", c: "var(--gold)", b: "Open tracker",
      m: allowance >= 0 ? `${nf(allowance)} kcal left · burn ${nf(burnLeft)} kcal today` : `${nf(-allowance)} kcal over target` },
    { k: "profile", n: "Profile", tag: "your stats", ic: "user", c: "var(--mint)", b: "Edit profile",
      m: `${prof.height} cm · ${prof.weight} kg · ${goalDef.l.toLowerCase()}` },
    { k: "shop", n: "Kit & shop", tag: "store", ic: "bag", c: "var(--heat)", b: bagCount ? `Bag · ${bagCount}` : "Browse the shop",
      m: `◆ ${nf(p.coins)} coins ready to spend` },
  ];

  const menuView = (
    <div className="mainmenu">
      {brandHead}
      <div className="reveal" style={{ animationDelay: "130ms", marginBottom: 12 }}>{rankCard}</div>

      <div style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
        <div className="reveal" style={{ animationDelay: "230ms", flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
          <div className="hcar" ref={carRef} onScroll={(e) => {
            const el = e.currentTarget;
            setCarI(Math.min(HOME.length - 1, Math.round(el.scrollLeft / (el.scrollWidth / HOME.length))));
          }}>
            {HOME.map((h) => (
              <div className="hcard" key={h.k}>
                <div className="hart"><Icon n={h.ic} c={h.c} s={54} /></div>
                <div>
                  <div className="eyebrow" style={{ color: h.c }}>{h.tag}</div>
                  <div className="ht" style={{ marginTop: 6 }}>{h.n}</div>
                  <div className="hm">{h.m}</div>
                </div>
                <button className={h.primary ? "cta" : "ghost"} onClick={() => setScreen(h.k)}
                  style={h.primary ? { fontFamily: "var(--display)", fontWeight: 800, letterSpacing: ".14em",
                    textTransform: "uppercase", fontSize: 15.5, padding: "14px 14px" } : null}>{h.b}</button>
              </div>
            ))}
          </div>

          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 9 }}>
            <button className="carnav" disabled={!HOME[carI - 1]}
              aria-label={HOME[carI - 1] ? "Previous: " + HOME[carI - 1].n : "Previous"}
              onClick={() => carRef.current && carRef.current.scrollBy({ left: -carRef.current.clientWidth, behavior: "smooth" })}>
              <span className="cn-g">◂</span>
              <span className="cn-l">
                <span className="cn-k">Prev</span>
                <span className="cn-n">{HOME[carI - 1] ? HOME[carI - 1].n : "—"}</span>
              </span>
            </button>
            <button className="carnav" disabled={!HOME[carI + 1]} style={{ justifyContent: "flex-end", textAlign: "right" }}
              aria-label={HOME[carI + 1] ? "Next: " + HOME[carI + 1].n : "Next"}
              onClick={() => carRef.current && carRef.current.scrollBy({ left: carRef.current.clientWidth, behavior: "smooth" })}>
              <span className="cn-l">
                <span className="cn-k">Next</span>
                <span className="cn-n">{HOME[carI + 1] ? HOME[carI + 1].n : "—"}</span>
              </span>
              <span className="cn-g">▸</span>
            </button>
          </div>
          <div className="dots" style={{ margin: "9px 0 0", justifyContent: "center" }}>
            {HOME.map((h, i) => <i key={h.k} data-on={carI === i ? 1 : 0} />)}
          </div>
        </div>
      </div>

      <div className="reveal" style={{ animationDelay: "660ms", display: "flex", justifyContent: "space-between",
        paddingTop: 12, fontFamily: "var(--mono)", fontSize: 9, letterSpacing: ".16em",
  ...
  const hubView = (
    <div className="body">
      <div className="sect-hd" style={{ marginBottom: 14 }}>
        <span className="eyebrow">{prof.name || "Player"} · {r.name} {r.div}</span><div className="rule" />
        <span className="eyebrow">lvl {lvl}</span>
      </div>
      {MENU.map((it, i) => (
        <div className="reveal" key={it.k} style={{ animationDelay: `${40 + i * 55}ms`, marginBottom: 9 }}>
      ...
      <p className="mono" style={{ fontSize: 9.5, color: "var(--dim)", textAlign: "center", marginTop: 16,
        letterSpacing: ".12em", textTransform: "uppercase" }}>Start with Train to run a session</p>
    </div>
  );

  const profileView = (() => {
    const stepb = { width: 38, height: 38, borderRadius: "50%", flexShrink: 0, fontSize: 16, color: "var(--ice)",
      background: "var(--glass)", border: "1px solid var(--line)", boxShadow: "var(--rim)" };
    const Num = ({ lbl, unit, val, on, min, max, step = 1 }) => (
      <Cut pad={13} style={{ marginTop: 0 }}>
        <div className="eyebrow">{lbl}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 9 }}>
          <button style={stepb} onClick={() => on(Math.max(min, val - step))} aria-label={`${lbl} down`}>−</button>
          <div className="mono" style={{ flex: 1, textAlign: "center", fontSize: 17, color: "var(--ice)" }}>
            {val}<span style={{ fontSize: 10, color: "var(--dim)" }}> {unit}</span>
          </div>
          <button style={stepb} onClick={() => on(Math.min(max, val + step))} aria-label={`${lbl} up`}>+</button>
        </div>
      </Cut>
    );
    return (
      <div className="body">
        <div className="sect">
          <div className="sect-hd"><span className="eyebrow">profile picture</span><div className="rule" /></div>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <Avatar i={prof.pfp} size={68} label={initial} />
            <div style={{ display: "flex", gap: 9, flexWrap: "wrap" }}>
              {PFP.map((c, i) => (
                <button key={i} onClick={() => setProf((v) => ({ ...v, pfp: i }))} aria-label={`Avatar ${i + 1}`}
                  style={{ width: 32, height: 32, borderRadius: "32%", background: `linear-gradient(140deg,${c[0]},${c[1]})`,
                    boxShadow: prof.pfp === i ? "0 0 0 2px var(--cyan)" : "0 6px 14px rgba(0,0,0,.4)" }} />
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="sect">
          <div className="sect-hd"><span className="eyebrow">display name</span><div className="rule" /></div>
          <input className="inp" value={prof.name} maxLength={14} placeholder="Your handle"
            onChange={(e) => { const v = e.target.value; setProf((q) => ({ ...q, name: v })); setP((q) => ({ ...q, handle: v || "Player" })); }} />
        </div>

        <div className="sect">
          <div className="sect-hd"><span className="eyebrow">body stats</span><div className="rule" /></div>
          <div className="grid">
            <Num lbl="Height" unit="cm" val={prof.height} min={120} max={220} on={(v) => setProf((q) => ({ ...q, height: v }))} />
            <Num lbl="Weight" unit="kg" val={prof.weight} min={35} max={180} on={(v) => setProf((q) => ({ ...q, weight: v }))} />
            <Num lbl="Age" unit="yrs" val={prof.age} min={13} max={80} on={(v) => setProf((q) => ({ ...q, age: v }))} />
            <Cut pad={13} style={{ marginTop: 0 }}>
              <div className="eyebrow">Sex</div>
              <div className="tabs2" style={{ marginTop: 9, marginBottom: 0 }}>
                <button data-on={prof.sex === "m" ? 1 : 0} onClick={() => setProf((q) => ({ ...q, sex: "m" }))}>M</button>
                <button data-on={prof.sex === "f" ? 1 : 0} onClick={() => setProf((q) => ({ ...q, sex: "f" }))}>F</button>
              </div>
            </Cut>
          </div>
        </div>
        ...
        <div className="sect">
          <div className="sect-hd"><span className="eyebrow">activity outside training</span><div className="rule" /></div>
          <div className="tabs2" style={{ marginBottom: 0 }}>
            {ACT.map(([v, l]) => (
              <button key={l} data-on={prof.act === v ? 1 : 0} onClick={() => setProf((q) => ({ ...q, act: v }))}>{l}</button>
            ))}
          </div>
        </div>

        <div className="sect">
          <div className="sect-hd"><span className="eyebrow">goal</span><div className="rule" /></div>
          <div className="tabs2" style={{ marginBottom: 0 }}>
            {Object.keys(GOALS).map((g) => (
              <button key={g} data-on={prof.goal === g ? 1 : 0} onClick={() => setProf((q) => ({ ...q, goal: g }))}>{GOALS[g].l}</button>
            ))}
          </div>
          <p className="mono" style={{ fontSize: 10, color: "var(--dim)", marginTop: 11, lineHeight: 1.8, letterSpacing: ".06em" }}>
            {goalDef.l} · {goalDef.r}
          </p>
        </div>

        <div className="sect">
          <div className="sect-hd"><span className="eyebrow">what this sets</span><div className="rule" /></div>
          <Cut>
            {[["Resting burn (BMR)", `${nf(bmr)} kcal`], ["Daily burn with activity", `${nf(tdee)} kcal`],
              ["Calorie target", `${nf(kcalTarget)} kcal`], ["Training burn goal", `${nf(burnGoal)} kcal / day`]].map(([k, v]) => (
              <div key={k} style={{ display: "flex", justifyContent: "space-between", gap: 12, padding: "9px 0",
                borderBottom: "1px solid var(--line2)" }}>
                <span className="mmeta">{k}</span>
                <span className="mono" style={{ fontSize: 12.5, color: "var(--cyan)" }}>{v}</span>
              </div>
            ))}
          </Cut>
        </div>

        <button className="cta" onClick={() => { flash("Profile saved"); setScreen("menu"); }}>Save profile</button>
      </div>
    );
  })();
  ...
  const nutriView = (
    <div className="body">
      <Cut>
        <div className="eyebrow">today · {allowance >= 0 ? "still available" : "over target"}</div>
        <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginTop: 6 }}>
          <div className="dsp" style={{ fontSize: 34, color: allowance >= 0 ? "var(--cyan)" : "var(--gold)" }}>
            {nf(Math.abs(allowance))}
          </div>
          <div className="eyebrow">kcal</div>
        </div>
        <div className="bar" style={{ marginTop: 12 }}>
          <i style={{ width: `${Math.min(100, (eaten / (kcalTarget + burned)) * 100)}%`,
            background: allowance >= 0 ? "linear-gradient(90deg,#8FEBFF,#2B7BFF)" : "linear-gradient(90deg,#FFD98A,#FF7A45)" }} />
        </div>
        <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
          {[["target", `${nf(kcalTarget)}`], ["eaten", `${nf(eaten)}`], ["burned", `${nf(burned)}`]].map(([k, v]) => (
            <div key={k} style={{ flex: 1 }}>
              <div className="mono" style={{ fontSize: 15, color: "var(--ice)" }}>{v}</div>
      ...
      <div className="sect">
        <div className="sect-hd"><span className="eyebrow">your daily plan</span><div className="rule" /></div>
        <Cut>
          <div className="mlbl" style={{ fontSize: 14 }}>Eat {nf(kcalTarget)} kcal · burn {nf(burnGoal)} kcal</div>
          <p className="mmeta" style={{ marginTop: 7, lineHeight: 1.75 }}>
            You burn about {nf(tdee)} kcal a day at {prof.weight} kg with {ACT.find(([v]) => v === prof.act)[1].toLowerCase()} activity.
            To {goalDef.l.toLowerCase()} you eat {nf(kcalTarget)} kcal and put {nf(burnGoal)} of that burn through training, {goalDef.r}.
          </p>
        </Cut>
      </div>

      <div className="sect">
        <div className="sect-hd"><span className="eyebrow">log what you ate</span><div className="rule" /></div>
        <div className="tabs2">
          <button data-on={nutriTab === "describe" ? 1 : 0} onClick={() => { setNutriTab("describe"); setPending(null); }}>Describe it</button>
          <button data-on={nutriTab === "photo" ? 1 : 0} onClick={() => { setNutriTab("photo"); setPending(null); }}>Photo</button>
        </div>

        {nutriTab === "describe" ? (
          <>
            <input className="inp" value={foodQ} placeholder="e.g. chicken wrap and a coke"
              onChange={(e) => setFoodQ(e.target.value)}
              onKeyDown={(e) => { if (e.key === "Enter") setPending(estimateFood(foodQ)); }} />
            <div style={{ height: 9 }} />
            <button className="ghost" disabled={!foodQ.trim()} onClick={() => setPending(estimateFood(foodQ))}>Estimate calories</button>
          </>
        ) : (
          <>
            <div style={{ position: "relative", borderRadius: 22, overflow: "hidden", height: 168,
              border: "1px solid var(--line)", background: "radial-gradient(120% 90% at 50% 40%, rgba(43,123,255,.16), rgba(5,8,15,0) 65%), var(--glass)",
              display: "flex", alignItems: "center", justifyContent: "center" }}>
              <div style={{ position: "absolute", inset: 22, border: "1px solid rgba(79,216,255,.35)", borderRadius: 14 }} />
              <div className="mono" style={{ fontSize: 10.5, color: scanning ? "var(--cyan)" : "var(--dim)",
                letterSpacing: ".16em", textTransform: "uppercase", textAlign: "center", padding: "0 30px", lineHeight: 1.9 }}>
                {scanning ? "Analysing photo…" : "Point the camera at your meal or drink"}
              </div>
            </div>
            <div style={{ height: 11 }} />
            <button className="ghost" disabled={scanning} onClick={takePhoto}>{scanning ? "Analysing…" : "Take photo"}</button>
          </>
        )}
        ...
        {pending && (
          <Cut gold>
            <div className="eyebrow" style={{ color: "var(--gold)" }}>
              {pending.src === "photo" ? `identified · ${pending.conf}% confidence` : pending.rough ? "rough estimate" : "matched"}
            </div>
            <div className="mlbl" style={{ fontSize: 15, marginTop: 6 }}>{pending.t}</div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12 }}>
              <button className="ghost" style={{ width: 44, padding: 11 }}
                onClick={() => setPending((v) => ({ ...v, kcal: Math.max(25, v.kcal - 25) }))} aria-label="Less">−</button>
              <div className="mono" style={{ flex: 1, textAlign: "center", fontSize: 19, color: "var(--cyan)" }}>
                {nf(pending.kcal)}<span style={{ fontSize: 10, color: "var(--dim)" }}> kcal</span>
              </div>
              <button className="ghost" style={{ width: 44, padding: 11 }}
                onClick={() => setPending((v) => ({ ...v, kcal: v.kcal + 25 }))} aria-label="More">+</button>
            </div>
            <div style={{ height: 11 }} />
            <button className="cta" onClick={() => logMeal(pending)}>Add to today</button>
          </Cut>
        )}
      </div>

      <div className="sect">
        <div className="sect-hd"><span className="eyebrow">logged today · {nf(eaten)} kcal</span><div className="rule" /></div>
        {meals.length === 0 ? (
          <div className="empty"><div className="dsp" style={{ fontSize: 14 }}>Nothing logged yet</div>
            <p>Describe a meal or photograph it to start the day.</p></div>
        ) : meals.map((m) => (
          <div className="fr" key={m.id}>
            <div className="av" style={{ color: "var(--dim)", fontSize: 10 }}>{m.src === "photo" ? "IMG" : "TXT"}</div>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontWeight: 600, fontSize: 13.5 }}>{m.t}</div>
              <div className="ex-k">{m.src === "photo" ? "from photo" : "described"}</div>
            </div>
            <div className="mono" style={{ marginLeft: "auto", fontSize: 12.5, color: "var(--cyan)" }}>{nf(m.kcal)}</div>
            <button className="buy" data-can="1" onClick={() => setMeals((ms) => ms.filter((x) => x.id !== m.id))}>Remove</button>
          </div>
        ))}
      </div>
      ...
      <div className="sect">
        <div className="sect-hd">
          <span className="eyebrow">{burnLeft > 0 ? `burn ${nf(burnLeft)} kcal to stay on plan` : "burn goal met"}</span>
          <div className="rule" />
        </div>
        {burnLeft > 0 ? (
          <>
            <p className="mmeta" style={{ margin: "0 0 12px", lineHeight: 1.75 }}>
              {allowance < 0
                ? `You are ${nf(-allowance)} kcal over target, plus ${nf(Math.max(0, burnGoal - burned))} kcal left on today's burn goal. Any of these clears it.`
                : `${nf(burnLeft)} kcal left on today's burn goal. Any of these clears it.`}
            </p>
            {BURNS.map((b) => {
              const mins = Math.max(4, Math.ceil(burnLeft / b.per));
              return (
                <div className="fr" key={b.t}>
                  <div className="av" style={{ color: "var(--cyan)", fontSize: 11 }}>{mins}′</div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontWeight: 600, fontSize: 13.5 }}>{b.t}</div>
                    <div className="ex-k">{b.kit} · {b.per} kcal/min · +{b.xp} XP</div>
                  </div>
                  <button className="buy" data-can="1" style={{ marginLeft: "auto" }} onClick={() => logBurn(b, mins)}>Log</button>
                </div>
              );
            })}
          </>
          ...
          <div className="empty">
            <div className="dsp" style={{ fontSize: 14 }}>Burn goal met</div>
            <p>{nf(burned)} kcal burned today. Anything else is a bonus.</p>
          </div>
        )}
      </div>

      <button className="cta" onClick={() => { setSets([]); setScreen("quests"); }}>Go to today's session</button>
    </div>
  );
      ...
      {!bag.length ? (
        ...
        <>
            ...
            return (
              <div className="bl" key={b.id}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 7, flexShrink: 0 }}>
                  ...
                  <button className="trash" aria-label={`Remove ${it.t}`}
                    onClick={() => setBag((x) => x.filter((y) => y.id !== b.id))}><Icon n="trash" s={15} /></button>
                </div>
              </div>
            );
          })}
          <div className="card" style={{ margin: "16px 0" }}>
            ...
            <div className="stat" style={{ borderTop: "1px solid var(--line)", marginTop: 4, paddingTop: 11 }}>
              <span style={{ color: "var(--ice)" }}>Total</span>
              <span className="dsp" style={{ fontSize: 17, color: "var(--cyan)" }}>${bagTotal}</span>
            </div>
          </div>
  ...
  /* ---------------------- registry ---------------------- */
  const SCREENS = {
    menu: menuView, hub: hubView, profile: profileView, nutri: nutriView, quests: questsView, session: sessionView, tracker: trackerView, summary: summaryView,
  ...
  const TITLES = {
    ...
    hub: "Play", profile: "Profile", nutri: "Calories",
  };
  const BACK = { session: "quests", tracker: "session", summary: "menu", product: "shop", bag: "shop",
    hub: "menu", profile: "menu", nutri: "menu", quests: "hub", rank: "hub", rewards: "hub", shop: "hub", friends: "hub" };
